...
#define Enum_String(Enumerator, String) \
    RegisterEnumerator(Enumerator, String);
//      }
...
public:
    ...
    static const std::string &ToString(const EnumType e);
...
typename EnumStringBase<D, E>::AssocMap &EnumStringBase<D, E>::GetMap()
{
    ...
    {
        bFirstAccess = false;
        D::RegisterEnumerators();               // 此处为了引用子类定义的方法，在模板类型中使用了class DerivedType，因为静态方法无法使用多态实现。
...
template <class D, class E>
const std::string &EnumStringBase<D, E>::ToString(const E e)
{
    for (;;) // Code block
    {
...
template <class DerivedType, class EnumType>
EnumType EnumStringBase<DerivedType, EnumType>::ToEnum(const std::string &str)
{
    ...
    return static_cast<EnumType>(GetMap().size() - 1);
}
