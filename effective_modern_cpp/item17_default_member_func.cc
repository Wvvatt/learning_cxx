...
public:
    ...
    2、Rule of Big Three：如果声明了拷贝构造、拷贝赋值操作符、析构三者其一，那么也应该声明其余两个 ——— 这代表用户接管了对自身资源的管理。

关于默认移动构造和移动赋值运算符的规则：
    1、如果显式定义了Big Three其中之一，编译器都不会再生成默认的移动构造或者移动赋值操作符。
    2、如果既没有显式定义移动，编译器也没有提供默认的，那么移动会转换为拷贝。因为移动实际上是请求移动，请求失败则拷贝。
    3、只有一个类没有显式定义Big Three、且每个非静态成员都可移动时，编译器才会生成默认的移动构造和移动赋值操作符。
    4、如果显式声明移动操作，那么两个默认拷贝操作不会再提供。
