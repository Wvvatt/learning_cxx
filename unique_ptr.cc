...
std::unique_ptr<D> pass_through_value(std::unique_ptr<D> p)
{
    p->bar();
    return p;
}

std::unique_ptr<D> pass_through_r_ref(std::unique_ptr<D> &&p)
{
    p->bar();
    return nullptr;
}
...
int main()
{
    std::cout << "1) Unique ownership semantics demo\n";
    {
        ...
        std::unique_ptr<D> q = pass_through_value(std::move(p));
 
        // p is now in a moved-from 'empty' state, equal to nullptr
        assert(!p);

        std::unique_ptr<D> n = pass_through_r_ref(std::move(q));
